...
import { listCountries } from "~/services/reference-data.server";
  ...
  const isActive =
    url.searchParams.get("status") === "active"
      ...
      : url.searchParams.get("status") === "inactive"
        ? false
        : undefined;
  const countries = await listCountries({ search, isActive });
  return { countries };
}
...
export default function CountriesListPage() {
  ...
  const basePath = `${basePrefix}/settings/references/countries`;
  ...
  return (
    <div className="space-y-6">
      ...
      {countries.length === 0 && !search && !status ? (
        <EmptyState
          icon={Globe}
