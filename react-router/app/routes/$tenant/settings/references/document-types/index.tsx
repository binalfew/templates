  ...
  const isActive =
    url.searchParams.get("status") === "active"
      ...
      : url.searchParams.get("status") === "inactive"
        ? false
        : undefined;
  const documentTypes = await listDocumentTypes({ search, isActive });
  return { documentTypes };
}

export default function DocumentTypesListPage() {
  const { documentTypes } = useLoaderData<typeof loader>();
  const [searchParams, setSearchParams] = useSearchParams();
  const basePrefix = useBasePrefix();
  const basePath = `${basePrefix}/settings/references/document-types`;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {documentTypes.length} document type
      ...
      {documentTypes.length === 0 && !search && !status ? (
        <EmptyState
          icon={FileText}
