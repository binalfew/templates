...
import { createTemplateSchema, MESSAGE_CHANNELS } from "~/utils/schemas/message-template";
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  try {
    await createTemplate(submission.value, ctx);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Define a new message template for notifications and broadcasts.
        </p>
      </div>
          ...
          <Form method="post" {...getFormProps(form)} className="space-y-4">
            ...
            <Field fieldId={fields.body.id} label="Body" required errors={fields.body.errors}>
              <Textarea
                ...
                key={fields.body.key}
                placeholder="Use {{varName}} for placeholders"
                rows={6}
              />
            </Field>
