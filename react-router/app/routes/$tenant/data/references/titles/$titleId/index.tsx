    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-bold text-foreground">{title.name}</h2>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" asChild className="w-full sm:w-auto">
            <Link to={`${basePath}/${title.id}/edit`}>
              <Pencil className="mr-1.5 size-3.5" />
              Edit
            </Link>
          </Button>
          <Button variant="outline" size="sm" asChild className="w-full sm:w-auto">
            <Link to={`${basePath}/${title.id}/delete`} className="text-destructive">
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span>{new Date(title.createdAt).toLocaleString()}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Updated</span>
            <span>{new Date(title.updatedAt).toLocaleString()}</span>
          </div>
        </CardContent>
      </Card>
    </div>
