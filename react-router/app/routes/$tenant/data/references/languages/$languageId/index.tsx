...
import { Languages, ArrowLeft, Pencil, Trash2 } from "lucide-react";
...
import { getLanguage } from "~/services/reference-data.server";
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" asChild className="w-full sm:w-auto">
            <Link to={`${basePath}/${language.id}/edit`}>
              <Pencil className="mr-1.5 size-3.5" />
              Edit
            </Link>
          </Button>
          <Button variant="outline" size="sm" asChild className="w-full sm:w-auto">
            <Link to={`${basePath}/${language.id}/delete`} className="text-destructive">
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span>{new Date(language.createdAt).toLocaleString()}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Updated</span>
            <span>{new Date(language.updatedAt).toLocaleString()}</span>
          </div>
        </CardContent>
      </Card>
    </div>
